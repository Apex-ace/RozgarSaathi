...
def single_face_check(image_path: str) -> dict:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Face detection failed: {exc}")
    if not faces:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No face detected in image")
    if len(faces) > 1:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Multiple faces detected. Use one face only")
    return {"faces": 1, "detector_backend": settings.DEEPFACE_DETECTOR_BACKEND}
...
def register_face_for_user(user_id: str, email: str, image_bytes: bytes, filename: str | None = None) -> dict:
    ...
    db = load_face_db()
    db[user_id] = {"email": email, "image_path": saved_path, "face_info": face_info}
    save_face_db(db)
    return {"user_id": user_id, "email": email, "image_path": saved_path, "face_info": face_info}


def verify_face_for_user(user_id: str, image_bytes: bytes, filename: str | None = None) -> dict:
    db = load_face_db()
    if user_id not in db:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No registered face found for this user")
    ...
    try:
        result = DeepFace.verify(
            img1_path=db[user_id]["image_path"],
            img2_path=live_path,
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Verification error: {exc}")
